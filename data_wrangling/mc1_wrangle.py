...
def normalize_nodes(nodes: pd.DataFrame) -> pd.DataFrame:
    ...
    # keep original name and stage name; build display name for convenience
...
def enrich_edges(edges: pd.DataFrame, nodes: pd.DataFrame) -> pd.DataFrame:
...
def build_works_master(nodes: pd.DataFrame, enriched: pd.DataFrame) -> pd.DataFrame:
    ...
    if not work_roles.empty:
        role_pivot = (
            work_roles
            .groupby(["target", "Edge Type"])["source_display_name"]
            .agg(join_unique)
            .unstack(fill_value="")
            .reset_index()
        ...
        role_count = (
            work_roles
            .groupby(["target", "Edge Type"])["source"]
            .nunique()
    ...
    if not label_roles.empty:
        label_pivot = (
            label_roles
            .groupby(["source", "Edge Type"])["target_name"]
            .agg(join_unique)
            .unstack(fill_value="")
            .reset_index()
        ...
        label_count = (
            label_roles
            .groupby(["source", "Edge Type"])["target"]
            .nunique()
            .unstack(fill_value=0)
            .reset_index()
            .rename(columns={"source": "id"})
    ...
    rename_map = {
        ...
        "notable": "notable",
        "single": "single",
        "PerformerOf_names": "performers",
        "ComposerOf_names": "composers",
        "LyricistOf_names": "lyricists",
        "ProducerOf_names": "producers",
        "RecordedBy_names2": "recorded_by_labels",
        "DistributedBy_names2": "distributed_by_labels",
        "PerformerOf_count": "num_performers",
        "ComposerOf_count": "num_composers",
        "LyricistOf_count": "num_lyricists",
        "ProducerOf_count": "num_producers",
        "RecordedBy_count2": "num_recorded_by_labels",
        "DistributedBy_count2": "num_distributed_by_labels",
    }
    out = out.rename(columns=rename_map)

    # influence counts touching each work
    ...
    for c in ["draws_from_count", "influenced_others_count",
              "num_performers","num_composers","num_lyricists","num_producers",
              "num_recorded_by_labels","num_distributed_by_labels"]:
        if c in out.columns:
            out[c] = out[c].fillna(0).astype("Int64")
    keep = [
        "id","work_name","work_type","genre","release_year","written_year","notoriety_year",
        "notable","single","is_oceanus_folk",
        "performers","composers","lyricists","producers",
        "num_performers","num_composers","num_lyricists","num_producers",
        "recorded_by_labels","distributed_by_labels",
        "num_recorded_by_labels","num_distributed_by_labels",
        "draws_from_count","influenced_others_count"
    ]
    for c in keep:
...
def build_contributors_bridge(enriched: pd.DataFrame) -> pd.DataFrame:
    df = enriched[enriched["Edge Type"].isin(WORK_ROLE_TYPES + LABEL_ROLE_TYPES)].copy()
    role_group = df["Edge Type"].map(lambda x: "label" if x in LABEL_ROLE_TYPES else "contributor")
    df["role_group"] = role_group
    out = df[[
        "Edge Type","role_group",
        "source","source_name","source_display_name","source_Node Type",
        "target","target_name","target_Node Type","target_genre","target_release_date"
    ]].rename(columns={
        "Edge Type":"edge_type",
        "source":"source_id","source_name":"source_name","source_display_name":"source_display_name","source_Node Type":"source_type",
        "target":"work_id","target_name":"work_name","target_Node Type":"work_type","target_genre":"work_genre","target_release_date":"work_release_year"
    })
    return out.sort_values(["work_release_year","work_id","edge_type","source_display_name"], na_position="last")

def build_influence_bridge(enriched: pd.DataFrame) -> pd.DataFrame:
    df = enriched[enriched["Edge Type"].isin(INFLUENCE_TYPES)].copy()
    out = df[[
        "Edge Type",
        "source","source_name","source_display_name","source_Node Type","source_genre","source_release_date",
        "target","target_name","target_display_name","target_Node Type","target_genre","target_release_date"
    ]].rename(columns={
        "Edge Type":"influence_type",
        "source":"source_id","source_name":"source_name","source_display_name":"source_display_name","source_Node Type":"source_type","source_genre":"source_genre","source_release_date":"source_year",
        "target":"target_id","target_name":"target_name","target_display_name":"target_display_name","target_Node Type":"target_type","target_genre":"target_genre","target_release_date":"target_year",
    })
    return out.sort_values(["source_year","target_year","influence_type"], na_position="last")

def build_memberof_bridge(enriched: pd.DataFrame) -> pd.DataFrame:
    df = enriched[enriched["Edge Type"].eq("MemberOf")].copy()
    out = df[[
        "source","source_name","source_display_name","source_Node Type",
        "target","target_name","target_display_name","target_Node Type"
    ]].rename(columns={
        "source":"person_id","source_name":"person_name","source_display_name":"person_display_name","source_Node Type":"person_type",
        "target":"group_id","target_name":"group_name","target_display_name":"group_display_name","target_Node Type":"group_type"
    })
    return out.sort_values(["group_name","person_display_name"])

def build_sailor_tables(nodes: pd.DataFrame, enriched: pd.DataFrame, works_master: pd.DataFrame):
    ...
    for _, r in direct_work_edges.iterrows():
        involvement_rows.append({
            "work_id": r["target"],
            "involvement_mode": "direct_person_edge",
            "role": r["Edge Type"],
            "linked_group": pd.NA
        })
    for _, r in group_performed_edges.iterrows():
        involvement_rows.append({
            "work_id": r["target"],
            "involvement_mode": "via_group_membership",
            "role": "PerformerOf",
            "linked_group": r["source_name"]
        })
    involvement = pd.DataFrame(involvement_rows)
    inv_agg = (
        involvement.groupby("work_id")
        .agg(
            sailor_involvement_modes=("involvement_mode", join_unique),
            sailor_roles=("role", join_unique),
            sailor_linked_groups=("linked_group", join_unique),
        )
        .reset_index()
    )
    ...
    # collaborators on those works (excluding Sailor herself and excluding labels)
    contrib = enriched[
        (enriched["target"].isin(sailor_work_ids)) &
        (enriched["Edge Type"].isin(WORK_ROLE_TYPES)) &
        (enriched["source"] != sailor_id)
    ].copy()
    collab = (
        contrib.groupby("target")
        .agg(
            collaborators=("source_display_name", join_unique),
            num_distinct_collaborators=("source", "nunique")
        )
        .reset_index()
        .rename(columns={"target":"id"})
    )
    ...
    # direct influences involving Sailor person OR Sailor-linked works
    inf = enriched[enriched["Edge Type"].isin(INFLUENCE_TYPES)].copy()
    sailor_inf = inf[
        (inf["source"].eq(sailor_id)) |
        (inf["target"].eq(sailor_id)) |
        (inf["source"].isin(sailor_work_ids)) |
        (inf["target"].isin(sailor_work_ids))
    ].copy()
    ...
    sailor_inf_out = sailor_inf[[
        ...
        "source":"source_id","source_name":"source_name","source_display_name":"source_display_name","source_Node Type":"source_type","source_genre":"source_genre","source_release_date":"source_year",
        "target":"target_id","target_name":"target_name","target_display_name":"target_display_name","target_Node Type":"target_type","target_genre":"target_genre","target_release_date":"target_year",
    }).sort_values(["direction","source_year","target_year"], na_position="last")

    return career, sailor_inf_out

def build_oceanus_tables(nodes: pd.DataFrame, enriched: pd.DataFrame):
    ...
    def direction(row):
        ...
        if src_of and not tgt_of:
            return "OF -> Other"
        if not src_of and tgt_of:
            return "Other -> OF"
        if src_of and tgt_of:
            return "OF -> OF"
        return "Other -> Other"
    ...
    # year summary using OF works only
    of_works = nodes[nodes["id"].isin(of_ids)].copy()
    yearly_release = (
        of_works.groupby("release_date")
        .agg(
            of_works_released=("id","nunique"),
            of_notable_works=("notable", lambda s: int(pd.Series(s).fillna(False).sum()))
        )
        .reset_index()
    ...
    out_counts = (
        of_inf[of_inf["direction"].eq("OF -> Other")]
        .groupby("source_release_date")
        .size()
        .rename("of_to_other_influence_edges")
        .reset_index()
        .rename(columns={"source_release_date":"year"})
    )
    in_counts = (
        of_inf[of_inf["direction"].eq("Other -> OF")]
        .groupby("target_release_date")
        .size()
        .rename("other_to_of_influence_edges")
        .reset_index()
        .rename(columns={"target_release_date":"year"})
    )
    ...
    return spread, year_summary

def build_artist_careers(nodes: pd.DataFrame, enriched: pd.DataFrame, influence_bridge: pd.DataFrame):
    ...
    perf_summary = (
        perf.groupby("source")
        .agg(
            num_performed_works=("target","nunique"),
            first_performance_year=("target_release_date","min"),
            last_performance_year=("target_release_date","max"),
            primary_genre=("target_genre", lambda s: first_valid(pd.Series(s).mode())),
            all_performance_genres=("target_genre", join_unique),
            num_notable_performed_works=("target_notable", lambda s: int(pd.Series(s).fillna(False).sum()))
        )
        .reset_index()
        .rename(columns={"source":"id"})
    )
    ...
    role_summary = (
        all_roles.groupby("source")
        .agg(
            num_all_contributed_works=("target","nunique"),
            all_role_types=("Edge Type", join_unique),
            num_distinct_collaborators=("target", "nunique")
        )
        .reset_index()
        .rename(columns={"source":"id"})
    )

    # influence received by works they performed
    work_owner = perf[["source","target"]].drop_duplicates().rename(columns={"source":"id","target":"work_id"})
    influence_received = (
        influence_bridge[influence_bridge["target_id"].isin(work_owner["work_id"])]
        .merge(work_owner, left_on="target_id", right_on="work_id", how="left")
        .groupby("id")
        .size()
        .rename("performed_work_influence_received")
        .reset_index()
    )
    ...
    # keep artists/groups with at least 3 performed works OR 3 total contributed works
    cond = (
        out["num_performed_works"].fillna(0).ge(3) |
        out["num_all_contributed_works"].fillna(0).ge(3)
    )
    out = out[cond].copy()
    ...
    out = out.rename(columns={
        ...
        "Node Type":"artist_type"
    })
    keep = [
        "artist_id","artist_name","artist_display_name","artist_type","stage_name",
        "num_performed_works","num_all_contributed_works","all_role_types",
        "primary_genre","all_performance_genres",
        "first_performance_year","last_performance_year","career_span_years",
